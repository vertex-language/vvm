// text.go
// Package text is the A64 debug disassembler: it renders a lowered
// aarch64.Program as a human-readable listing by decoding each instruction
// word back into a mnemonic, using the same isa/aarch64 tables
// isa/aarch64/encoder assembled it from (see the format README's "asm —
// encode-only" section).
//
// There is no Decode here and none is planned: this package is scoped to
// exactly the encoding subset lower/aarch64's Encode switch (encode.go,
// isel.go, and friends) emits, not to A64 in general. A word outside that
...
func Encode(p *lowera64.Program) ([]byte, error) {
...
// whole number of 4-byte instructions (isa/aarch64.InstrBytes): a length that
// isn't is not a malformed instruction stream, it's a malformed Program,
// so it's reported rather than silently truncated.
func writeFunc(b *strings.Builder, f lowera64.Func) error {
	if len(f.Code)%4 != 0 {
	...
	for off := 0; off < len(f.Code); off += 4 {
		...
		if fxp != nil && !usedFixup {
			// Expected for things like load/store LO12 fixups that 
			// don't resolve neatly into the basic decode strings.
...
// in the target's final byte order (aarch64_be's data-word big-endianness
...
func writeGlobal(b *strings.Builder, g lowera64.Global) {
...
func relocComment(f lowera64.Fixup) string {
	if f.Addend == 0 {
...
func fixupsByOffset(fx []lowera64.Fixup) map[uint32]lowera64.Fixup {
	m := make(map[uint32]lowera64.Fixup, len(fx))
	for _, f := range fx {
		m[f.Offset] = f
	}
	return m
