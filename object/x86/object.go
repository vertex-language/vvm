// Package object translates a lowered x86.Program into a generic,
// container-agnostic description of sections, symbols, and relocations —
// arrow 4 of the README taxonomy.
//
// "Generic" means: shaped the way essentially any .o-builder wants its input
// (byte-offset fixups, named symbols, text/data/bss section kinds), but
// expressed in this package's own types. This package does not import
// objectfile, and knows nothing about ELF/COFF/Mach-O; the only arch-specific
// knowledge it adds is the x86.FixupKind -> RelocKind mapping, which for
...
const (
	// RelocPCRel32: field := S + A - P (i386 R_386_PC32 shape).
	RelocPCRel32 RelocKind = iota
	// RelocAbs32: field := S + A (i386 R_386_32 shape).
	RelocAbs32
)
...
func relocKind(k x86.FixupKind) RelocKind {
	if k == x86.FixupPCRel32 {
		return RelocPCRel32
	}
	return RelocAbs32
}
...
// initialized globals go to data, zero globals to bss, with tdata/tbss for
// TLS. Fixup offsets are rebased to section offsets and mapped to RelocKinds.
func FromProgram(p *x86.Program) []Section {
	...
	place := func(sec *Section, g x86.Global, withData bool) {
