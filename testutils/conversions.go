...
func init() {
	register(testCase{
		...
		build: func() *vir.Module {
			return i32PrintingModule("convert_trunc", func(fb *vir.FunctionBuilder) vir.Operand {
	...
	register(testCase{
		...
		build: func() *vir.Module {
			return i32PrintingModule("convert_sext", func(fb *vir.FunctionBuilder) vir.Operand {
	...
	register(testCase{
		...
		build: func() *vir.Module {
			return i32PrintingModule("convert_zext", func(fb *vir.FunctionBuilder) vir.Operand {
	...
	register(testCase{
		...
		build: func() *vir.Module {
			return i32PrintingModule("convert_ptr_bitcast", func(fb *vir.FunctionBuilder) vir.Operand {
				p := fb.Alloca("p", vir.IntLiteral(4), 0)
				asInt := fb.Emit("as_int", vir.OpBitcast, vir.I64, p) // x86_64/aarch64 usize = 64 bits
