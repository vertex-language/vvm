...
// field alignment. Natural alignment holds inside aggregates, and i128
// aligns to 16.
type Layout struct {
	m       *vir.Module
...
func (l *Layout) AlignOf(t vir.Type) (int, error) {
	...
	case vir.IntType, vir.FloatType, vir.PtrType:
		return l.Size(t) // natural alignment, incl. 16 for i128
	case vir.VecType:
		sz, err := l.Size(t)
		if err != nil {
			return 0, err
...
// structOf returns the declared struct definition for name, for callers
// (globals.go's aggregate-initializer walk) that need the field list
// itself rather than just the computed offsets StructLayout returns.
func (l *Layout) structOf(name string) (*vir.Struct, error) {
	s, ok := l.structs[name]
	if !ok {
